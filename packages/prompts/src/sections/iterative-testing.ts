...
2. If the file exists (content is returned):
   ...
   - For small changes: Use targeted edits with specific line numbers (token-efficient)
   - For extensive changes: Rewrite the entire file
...
- Use targeted edits to update specific test case lines (token-efficient)
...
- Use targeted edits to insert new test cases at specific line positions
...
1. **Start with ONE test case** - Write the simplest, most fundamental test case first
   ...
   - Create the test file with just this ONE test case
...
3. **AFTER first test passes, batch related tests** - Write a describe block or test group at a time
   ...
   - Use targeted edits to insert the new describe block at the appropriate line position
...
- Write all test cases in a single file creation call (unless it's a new file)
...
- Use targeted edits to add describe blocks incrementally (token-efficient for large files)
</iterative_test_creation>`,
  );
