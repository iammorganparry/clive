...
export function getCurrentIteration(): { current: number; max: number } | null {
  const iterationFiles = [
    {
      state: path.join(CLAUDE_DIR, ".build-iteration"),
      max: path.join(CLAUDE_DIR, ".build-max-iterations"),
    },
    {
      state: path.join(CLAUDE_DIR, ".test-loop-state"),
      max: path.join(CLAUDE_DIR, ".test-max-iterations"),
    },
  ];
...
export function isCancellationPending(): boolean {
  return (
    fs.existsSync(path.join(CLAUDE_DIR, ".cancel-test-loop")) ||
...
export function isLockFilePresent(): boolean {
  return (
    fs.existsSync(path.join(CLAUDE_DIR, ".build-lock")) ||
