...
// Error types
export class TaskServiceConfigError extends Data.TaggedError(
  "TaskServiceConfigError",
)<{
...
export function createTaskService(config: Config) {
  ...
  const serviceLayer =
    config.issueTracker === "linear" && config.linear
    ...
    loadSessions: provide(
      Effect.gen(function* () {
        ...
        Effect.catchAll((error) => {
          ...
          return Effect.fail(
            new TaskServiceConfigError({
              message: `Failed to load sessions: ${errorMsg}`,
            }),
          );
        }),
      ),
    ...
    loadReadyTasks: provide(
      Effect.gen(function* () {
        ...
        Effect.catchAll((error) =>
          Effect.fail(
            new TaskServiceConfigError({
              message: `Failed to load ready tasks: ${error}`,
            }),
          ),
        ),
      ),
      ...
      provide(
        Effect.gen(function* () {
          if (config.issueTracker === "linear" && config.linear) {
            ...
            const states = yield* linearService.listWorkflowStates(
