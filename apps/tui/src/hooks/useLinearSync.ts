    ...
    try {
      ...
      if (hasChanges) {
        debugLog(
          "useLinearSync",
          isFirstSync
            ? "Initial sync - forcing cache update"
            : "Changes detected",
        );
      ...
      debugLog("useLinearSync", "Error polling Linear", {
        error: String(error),
      });
      return false;
    }
  ...
  useEffect(() => {
    ...
    const handleSignal = () => {
      if (fs.existsSync(signalPath)) {
        try {
          ...
          debugLog("useLinearSync", "Error processing signal file", {
            error: String(error),
          });
        }
      }
    ...
    try {
      ...
      if (fs.existsSync(dir)) {
        signalWatcher = fs.watch(dir, (_eventType, filename) => {
