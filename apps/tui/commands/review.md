...
allowed-tools: Bash, Read, Glob, Grep, mcp__linear__create_issue, mcp__linear__update_issue, mcp__linear__get_issue, mcp__linear__list_issues, mcp__linear__list_teams, mcp__linear__get_team, mcp__linear__list_issue_labels, mcp__linear__create_issue_label, mcp__linear__list_issue_statuses, mcp__linear__get_issue_status, mcp__linear__list_projects, mcp__linear__get_project, mcp__linear__search_documentation, mcp__plugin_playwright_playwright__*
denied-tools: Write, Edit, TodoWrite, Task, EnterPlanMode, ExitPlanMode, Skill
---
...
**Read login instructions from project CLAUDE.md:**
```bash
# Find auth/login instructions in the project's CLAUDE.md
cat CLAUDE.md 2>/dev/null | head -200
cat .claude/CLAUDE.md 2>/dev/null | head -200
```

Look for "Using the App", "Agent Browser Access", "Login", or "Authentication" sections. These tell you:
- Dev server URL and port
- Test user credentials or storage state file paths
- Step-by-step login flow

**Also check for saved session credentials:**
```bash
cat .claude/session-context.json 2>/dev/null | jq '.reviewCredentials'
```

**Browser Testing Workflow:**

1. **Navigate to app:**
   Use `mcp__plugin_playwright_playwright__browser_navigate` with the URL from CLAUDE.md

2. **Take initial snapshot:**
   Use `mcp__plugin_playwright_playwright__browser_snapshot` to capture page state

3. **If authentication required — follow CLAUDE.md instructions:**
   - Navigate to the sign-in page documented in CLAUDE.md
   - Use `mcp__plugin_playwright_playwright__browser_snapshot` to find form fields
   - Use `mcp__plugin_playwright_playwright__browser_fill_form` with test credentials from CLAUDE.md
   - Click sign-in button
   - `browser_snapshot` to verify login succeeded (not on sign-in page anymore)
   - Handle onboarding/org selection if prompted
