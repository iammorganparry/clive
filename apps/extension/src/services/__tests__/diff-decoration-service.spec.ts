...
import type { LineChange } from "../ai-agent/tools/diff-engine";
...
describe("DiffDecorationService", () => {
  ...
  beforeEach(async () => {
    ...
    mockComputeLineDiff = diffEngine.computeLineDiff as ReturnType<typeof vi.fn>;
  ...
  describe("applyDiffDecorations", () => {
    it.effect("should apply decorations for a new file (all lines green)", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          ...
          const [_decorationType, ranges] = mockEditor.setDecorations.mock.calls[0];
    ...
    it.effect("should apply decorations for added lines using Myers diff", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          expect(mockComputeLineDiff).toHaveBeenCalledWith(originalContent, newContent);
          ...
          const [_decorationType, addedRanges] = mockEditor.setDecorations.mock.calls[0];
    ...
    it.effect("should apply decorations for removed lines using Myers diff", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          ...
          const [_decorationType, removedRanges] = mockEditor.setDecorations.mock.calls[1];
    ...
    it.effect("should handle mixed added and removed lines", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          ...
          const [_addedDecorationType, addedRanges] = mockEditor.setDecorations.mock.calls[0];
          expect(addedRanges).toHaveLength(1);
          expect(addedRanges[0].start.line).toBe(1);
          expect(addedRanges[0].end.line).toBe(2); // 1 + 2 - 1
          
          // Check removed decorations
          const [_removedDecorationType, removedRanges] = mockEditor.setDecorations.mock.calls[1];
          expect(removedRanges).toHaveLength(1);
    ...
    it.effect("should clear decorations for a specific file", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          ...
          const [_clearAddedType, clearAddedRanges] = mockEditor.setDecorations.mock.calls[2];
          const [_clearRemovedType, clearRemovedRanges] = mockEditor.setDecorations.mock.calls[3];
  ...
  describe("getDecorationState", () => {
    ...
    it.effect("should return undefined for non-existent file", () =>
      Effect.gen(function* () {
        const service = yield* DiffDecorationService;
        const state = yield* service.getDecorationState("/non/existent/file.ts");
  ...
  describe("clearAllDecorations", () => {
    it.effect("should clear decorations for all files", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.sync(() => {
          (vscode.window as any).visibleTextEditors = [mockEditor1, mockEditor2];
        });
