...
describe("Diff View Integration", () => {
  ...
  it.effect("should use diff view when provider is available (new file)", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.sync(() => { currentMockInstance = mockDiffViewInstance; });
      ...
      const tool = yield* Effect.sync(() => createWriteTestFileTool(
        approvalRegistry,
        undefined,
        mockDiffProvider,
        false,
  ...
  it.effect("should use diff view when provider is available (existing file with overwrite)", () =>
    Effect.gen(function* () {
  ...
  it.effect("should handle user rejection in diff view", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.sync(() => {
        ...
        expect(mockDiffViewInstance.reset).toHaveBeenCalled();
      });
    }),
  );

  it.effect("should return rejection message when user rejects", () =>
    Effect.gen(function* () {
      yield* Effect.sync(() => approvalRegistry.add("test-diff-4"));
      ...
      yield* Effect.sync(() => {
        expect(result.success).toBe(false);
        expect(result.message).toContain("were rejected by user");
      });
    }),
  );

  it.effect("should handle diff view open failure gracefully", () =>
    Effect.gen(function* () {
      yield* Effect.sync(() => approvalRegistry.add("test-diff-5"));
  ...
  it.effect("should fallback to direct write when diffProvider not provided", () =>
    Effect.gen(function* () {
      yield* Effect.sync(() => approvalRegistry.add("test-diff-6"));

      // No diffProvider - should use direct file write
      const tool = yield* Effect.sync(() => createWriteTestFileTool(
        approvalRegistry,
        undefined,
        undefined, // No diffProvider
        false,
      ));

      const input: WriteTestFileInput = {
        proposalId: "test-diff-6",
        testContent: "test content",
        targetPath: "src/test.spec.ts",
      };
      ...
      yield* Effect.sync(() => {
        expect(result.success).toBe(true);
        expect(mockFs.writeFile).toHaveBeenCalled();
        // Should not call showInformationMessage for approval in direct write mode
