...
export class GitignoreReader {
	 ...
	 */
	private async readGitignoreFile(
	 ...
	 */
	private convertToCypressPatterns(
	 ...
	 */
	async getGitignorePatternsForCypress(
		workspaceRoot: vscode.Uri
	): Promise<string[]> {
		const gitignoreFiles = await this.findGitignoreFiles(workspaceRoot);
		...
		for (const gitignoreFile of gitignoreFiles) {
			try {
				const patterns = await this.readGitignoreFile(
					gitignoreFile,
					workspaceRoot
				);
				const cypressPatterns = this.convertToCypressPatterns(
					patterns,
					gitignoreFile
				);
...
export async function findGitignoreFiles(
	...
	const reader = new GitignoreReader();
	return reader.findGitignoreFiles(workspaceRoot);
}
...
export async function getGitignorePatternsForCypress(
	...
	const reader = new GitignoreReader();
	return reader.getGitignorePatternsForCypress(workspaceRoot);
}
