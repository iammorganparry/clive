...
/**
 * Create a mock VSCodeService layer for testing
 * Uses the createVSCodeMock factory to provide a mocked vscode module
 *
 * @example
 * ```typescript
 * const { layer, mockVscode } = createMockVSCodeServiceLayer();
 *
 * // Use in Effect programs
 * await Effect.gen(function* () {
 *   const service = yield* VSCodeService;
 *   const root = yield* service.getWorkspaceRoot();
 * }).pipe(Effect.provide(layer), Runtime.runPromise(runtime));
 *
 ...
 * ```
 */
export function createMockVSCodeServiceLayer(overrides?: VSCodeMockOverrides): {
  layer: Layer.Layer<VSCodeService>;
  mockVscode: typeof vscode;
} {
  // Use singleton instance if available (from vi.mock), otherwise create new one
  const mockVscode = getVSCodeMock() ?? createVSCodeMock(overrides);

  const layer = Layer.effect(
    VSCodeService,
    Effect.gen(function* () {
      return {
        _tag: "VSCodeService" as const,
        workspace: mockVscode.workspace,
        window: mockVscode.window,
        Uri: mockVscode.Uri,
        WorkspaceEdit: mockVscode.WorkspaceEdit,
        ...
        > =>
          pipe(
            Effect.sync(() => mockVscode.workspace.workspaceFolders),
            Effect.map((folders) =>
              Option.fromNullable(
        ...
        asRelativePath: (
          ...
          mockVscode.workspace.asRelativePath(uri, includeWorkspaceFolder),

        openTextDocument: (
          uri: vscode.Uri,
        ): Effect.Effect<vscode.TextDocument, DocumentError> =>
          Effect.tryPromise({
            try: () => mockVscode.workspace.openTextDocument(uri),
            catch: (error) =>
              new DocumentError({
        ...
        applyEdit: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.applyEdit(edit),
            catch: (error) =>
              new DocumentError({
        ...
        stat: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.fs.stat(uri),
            catch: (error) =>
              new FileSystemError({
        ...
        createDirectory: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.fs.createDirectory(uri),
            catch: (error) =>
              new FileSystemError({
        ...
        writeFile: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.fs.writeFile(uri, content),
            catch: (error) =>
              new FileSystemError({
        ...
        readFile: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.fs.readFile(uri),
            catch: (error) =>
              new FileSystemError({
        ...
        findFiles: (
          ...
          Effect.tryPromise({
            try: () =>
              mockVscode.workspace.findFiles(include, exclude, maxResults),
            catch: (error) =>
              new FileSystemError({
        ...
        isDirectory: (
          ...
          pipe(
            ...
            Effect.map((stat) => {
              ...
              const FileType = { Directory: 2, File: 1 } as const;
        ...
        deleteFile: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.workspace.fs.delete(uri, options),
            catch: (error) =>
              new FileSystemError({
        ...
        // Uri Utilities
        fileUri: (path: string): vscode.Uri => mockVscode.Uri.file(path),

        joinPath: (base: vscode.Uri, ...paths: string[]): vscode.Uri =>
          mockVscode.Uri.joinPath(base, ...paths),
        ...
        resolvePathToUri: (
          ...
          Effect.sync(() => {
            ...
            if (path.isAbsolute(filePath)) {
              return mockVscode.Uri.file(filePath);
            }
            return mockVscode.Uri.joinPath(workspaceRoot, filePath);
          }),
        ...
        showTextDocument: (
          ...
          Effect.tryPromise({
            try: () => mockVscode.window.showTextDocument(document, options),
            catch: (error) =>
              new DocumentError({
        ...
        createWorkspaceEdit: (): vscode.WorkspaceEdit =>
          new mockVscode.WorkspaceEdit(),

        createRange: (
          start: vscode.Position,
          end: vscode.Position,
        ): vscode.Range => new mockVscode.Range(start, end),
      };
    }),
  );
