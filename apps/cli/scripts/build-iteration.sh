...
# Build the execution prompt
{
    ...
    if [ "$LINEAR_TASK_FETCH" = true ]; then
        if [ -n "$CACHED_TASKS_FILE" ]; then
            echo "## CRITICAL: Tasks Pre-Loaded (DO NOT FETCH FROM LINEAR)"
            echo ""
            echo "ALL tasks for this epic are provided below. DO NOT call Linear MCP to fetch tasks."
            echo "DO NOT call mcp__linear__list_issues. DO NOT search for tasks."
            echo "Everything you need is in this JSON array."
            echo ""
            echo "## Available Tasks (from TUI cache)"
            echo ""
            echo "\`\`\`json"
            ...
            echo ""
            echo "1. Look at the JSON array above"
            echo "2. Find tasks where Status is 'pending' or 'in_progress'"
            echo "3. Select the first pending task (they're already ordered)"
            echo "4. Use the task's ID field for mcp__linear__update_issue calls"
            echo "5. Use the task's Title field for display and commit messages"
            echo ""
            echo "**After completing a task:**"
            echo ""
            echo "- Update the task status in Linear: mcp__linear__update_issue (id: [task-id], state: 'Done')"
            echo "- Move to the next pending task from the JSON above"
            echo "- DO NOT fetch tasks again - use the JSON provided"
            echo ""
            echo "**If all tasks in the JSON are complete:**"
            echo ""
    ...
    echo ""
    echo "## Output Formatting for TUI"
    echo ""
    echo "You are running in a Terminal UI that cannot render Markdown."
    echo ""
    echo "**IMPORTANT formatting rules:**"
    echo "- Output PLAIN TEXT, not Markdown"
    echo "- Do NOT use: code blocks (\`\`\`), bold (**text**), italics (*text*), headers (##)"
    echo "- Do NOT use: lists with dashes (-), numbered lists (1. 2. 3.)"
    echo "- Use simple text with blank lines for spacing"
    echo "- Use CAPITAL LETTERS or [brackets] for emphasis instead of markdown"
    echo "- Example GOOD: \"Checking file status...\" followed by blank line"
    echo "- Example BAD: \"## Checking Status\" or \`\`\`bash code \`\`\`"
    echo ""
    echo "Keep output concise and readable in a plain terminal."
    echo ""
