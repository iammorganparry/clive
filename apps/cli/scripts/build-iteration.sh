...
# Build the execution prompt
{
    ...
    if [ "$LINEAR_TASK_FETCH" = true ]; then
        if [ -n "$CACHED_TASKS_FILE" ]; then
            echo "## Task List (from cache)"
            echo ""
            echo "Available tasks in this epic:"
            echo ""
            echo "\`\`\`json"
            cat "$CACHED_TASKS_FILE"
            echo "\`\`\`"
            echo ""
            echo "**Select the next task to work on:**"
            echo ""
            echo "1. Find tasks with status 'pending' or 'in_progress'"
            echo "2. Select the oldest/highest priority task (lowest tier number)"
            echo "3. Use the task's \`id\` field for MCP calls"
            echo "4. Use the task's \`title\` field for display/commits"
            echo ""
            echo "**After completing a task:**"
            echo ""
            echo "- Update the task status in Linear using \`mcp__linear__update_issue\`"
            echo "- Move to the next pending task from the list above"
            echo ""
